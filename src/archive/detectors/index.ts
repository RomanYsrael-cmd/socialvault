...
export function detectArchiveSet(parts: ArchiveEntryInfo[][]): DetectionResult {
  ...
  return { metadataOnlyEligible: results.length > 0 && results.every(result => result.metadataOnlyEligible), supported, platform: supported ? 'facebook' : 'unknown', confidence: supported ? Math.min(.99, .58 + sections.length * .05) : format === 'html' && sections.length ? .2 : .03, entryCount: results.reduce((sum, result) => sum + result.entryCount, 0), inspectedEntries: results.reduce((sum, result) => sum + result.inspectedEntries, 0), sections, supportedSections, unsupportedSections, warnings, format };
}
