...
class ObjectUrlCache {
  ...
  private clock = 0;
  async acquire(file: File, path: string, mimeType?: string, archivePartId?: string) {
    const key = `${archivePartId ?? ''}:${file.name}:${file.size}:${file.lastModified}:${path}`;
    let entry = this.entries.get(key);
    if (!entry) { entry = { key, refs: 0, touched: ++this.clock }; this.entries.set(key, entry); }
    entry.refs++; entry.touched = ++this.clock;
    if (!entry.url) {
      entry.pending ??= mediaClient.extract(file, path, mimeType, archivePartId).then(blob => { const url = URL.createObjectURL(blob); if (this.entries.get(key) !== entry) { URL.revokeObjectURL(url); throw new Error('Media cache was cleared.'); } entry!.url = url; entry!.pending = undefined; this.evict(); return url; }).catch(error => { entry!.pending = undefined; throw error; });
      try { return await entry.pending; } catch (error) { this.release(key); throw error; }
    }
    return entry.url;
  }
  releaseKey(file: File, path: string, archivePartId?: string) { this.release(`${archivePartId ?? ''}:${file.name}:${file.size}:${file.lastModified}:${path}`); }
