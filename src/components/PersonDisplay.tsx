...
export type PersonDisplayValue = { id?: string; displayName?: string; photoPath?: string; archivePartId?: string };
...
export function PersonDisplay({ person, context, compact = false }: { person: PersonDisplayValue; context?: string; compact?: boolean }) {
  ...
  const media = person.photoPath ? { id: `person-photo:${person.id ?? name}`, path: person.photoPath, mediaType: 'photo' as const, ownerType: 'post' as const, ownerId: person.id ?? name, source: { platform: 'facebook', path: 'profile', archivePartId: person.archivePartId } } satisfies Media : undefined;
