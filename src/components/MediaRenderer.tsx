import { AlertCircle, FileText, LoaderCircle, Paperclip, Play } from 'lucide-react';
...
export function MediaRenderer({ media, onOpen, compact = false }: { media: Media; onOpen?: () => void; compact?: boolean }) {
  const { url, error } = useMediaObjectUrl(media);
  const unavailable = error || 'Reconnect the matching archive to view this media.';
  ...
  if (!url) return <button type="button" className={`media-placeholder ${compact ? 'compact' : ''}`} onClick={onOpen} disabled={!!error}><span>{error ? <AlertCircle/> : <LoaderCircle className="spin"/>}</span><b>{error ? 'Media unavailable' : 'Loading media…'}</b><small>{unavailable}</small></button>;
