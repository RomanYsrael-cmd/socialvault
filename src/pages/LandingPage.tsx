...
export function LandingPage() {
  ...
  const inspect = () => {
    ...
    worker.onmessage = (event: MessageEvent<ImportProgress>) => { const message = event.data; if (message.type === 'progress') setStatus(message.message); if (message.type === 'error') { setError(message.message); setBusy(false); worker.terminate(); } if (message.type === 'result') { setBusy(false); worker.terminate(); if (!message.result.supported) { setError(message.result.format === 'html' && message.result.sections.length ? 'Facebook HTML markers were found, but no supported structural pages were detected. Choose a complete export set and try again.' : message.result.format === 'mixed' ? 'This archive mixes Facebook JSON and HTML files, but no supported Facebook structure was detected.' : 'These ZIPs do not look like a supported Facebook Download Your Information archive.'); return; } save(files, message.result); nav('/archive'); } };
