import type{ActivityRecord,ActivityType,Album,Comment,Connection,Conversation,Media,Message,NormalizedArchiveData,Person,Post,Profile,ArchiveIdentity}from'../archive/schemas/models';
...
export type DatabaseRequest={id:number;type:'init'|'replace'|'profile'|'people'|'person'|'posts'|'post'|'conversations'|'messages'|'search'|'stats'|'media'|'connections'|'albums'|'album'|'memories'|'activity'|'archive-identity'|'media-cache';data?:NormalizedArchiveData;conversationId?:string;personId?:string;postId?:string;albumId?:string;cursor?:string;limit?:number;sort?:'newest'|'oldest';year?:number;month?:number;day?:number;query?:string;activityType?:ActivityType;connectionType?:Connection['type'];mediaType?:Media['mediaType'];mediaPath?:string;byteSize?:number};
export type DatabaseResult=Profile|PersonSummary|PersonSummary[]|Post|Post[]|Comment[]|ConnectionSummary[]|AlbumSummary|AlbumSummary[]|AlbumDetail|ConversationPreview[]|Message[]|Media[]|MemoryRecord[]|ActivityRecord[]|Page<Post>|Page<PersonSummary>|Page<ConversationPreview>|Page<Message>|Page<Media>|Page<ConnectionSummary>|Page<AlbumSummary>|Page<MemoryRecord>|Page<ActivityRecord>|SearchResponse|ArchiveStats|ArchiveIdentity|{mode:StorageMode;searchBackend:SearchBackend}|undefined;
