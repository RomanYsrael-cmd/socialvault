...
function replace(input: NormalizedArchiveData) {
  ...
  db.transaction(() => {
    ...
    batchInsert('posts', ['id', 'author_id', 'title', 'body', 'created_at', 'source_path', 'source_index', 'archive_part_id'], data.posts.map(post => [post.id, post.authorId ?? null, post.title ?? null, post.text ?? null, post.createdAt ?? null, post.source.path, post.source.index ?? null, partIdFor(post.source)]));
...
function importPartData(dataInput: NormalizedArchiveData, part: ArchivePart, sessionId: string): ImportPartCheckpoint {
  const data = normalizedData(dataInput); const now = new Date().toISOString();
  const existing = currentImportState(sessionId).checkpoints.find(checkpoint => checkpoint.archivePartId === part.id);
  ...
  // it with a one-part fallback for every checkpoint.
  const savedArchiveSet = archiveSetFromRow(rows('SELECT id,platform,created_at createdAt,part_count partCount,total_size totalSize,fingerprint,imported_at importedAt,status FROM archive_sets WHERE id=? LIMIT 1', [part.archiveId])[0]) ?? archiveSetFromRow(rows('SELECT id,platform,created_at createdAt,part_count partCount,total_size totalSize,fingerprint,imported_at importedAt,status FROM archive_sets ORDER BY created_at DESC LIMIT 1')[0]);
  const archiveSet = data.archiveSet ?? savedArchiveSet ?? { id: part.archiveId, platform: 'facebook' as const, createdAt: Date.now(), partCount: 1, totalSize: part.fileSize, fingerprint: part.manifestFingerprint, status: 'incomplete' as const };
  const activities = activityForData(data); recordCounts.activities = activities.length;
  db.transaction(() => {
    db.exec({ sql: 'INSERT OR IGNORE INTO archive_sets(id,platform,created_at,part_count,total_size,fingerprint,imported_at,status) VALUES(?,?,?,?,?,?,?,?)', bind: [archiveSet.id, archiveSet.platform, archiveSet.createdAt, archiveSet.partCount, archiveSet.totalSize, archiveSet.fingerprint, null, 'incomplete'] });
    ...
    db.exec({ sql: 'INSERT OR REPLACE INTO archive_parts(id,archive_id,part_index,filename,file_size,entry_count,manifest_fingerprint,connected,status,warning_count,sections) VALUES(?,?,?,?,?,?,?,?,?,?,?)', bind: [part.id, archiveSet.id, part.partIndex, part.filename, part.fileSize, part.entryCount, part.manifestFingerprint, part.connected ? 1 : 0, 'complete', partWarningCount, JSON.stringify(part.sections ?? data.importedSections ?? [])] });
    if (data.profile) {
    ...
    batchInsert('posts', ['id', 'author_id', 'title', 'body', 'created_at', 'source_path', 'source_index', 'archive_part_id'], data.posts.map(post => [post.id, post.authorId ?? null, post.title ?? null, post.text ?? null, post.createdAt ?? null, post.source.path, post.source.index ?? null, part.id]), 'OR IGNORE');
    ...
    // A later ZIP part may contain the binary for a media reference that was
    // normalized while importing an earlier part. Match the complete path so
    // resume can repair the source without rereading old JSON.
    for (const item of data.media) db.exec({ sql: 'UPDATE media SET archive_part_id=? WHERE path=?', bind: [item.source.archivePartId ?? part.id, item.path] });
  ...
  const state = currentImportState(sessionId); if (state.session) {
  ...
  const persistedSet = archiveSetFromRow(rows('SELECT id,platform,created_at createdAt,part_count partCount,total_size totalSize,fingerprint,imported_at importedAt,status FROM archive_sets ORDER BY created_at DESC LIMIT 1')[0]);
  const persistedParts = rows('SELECT id,archive_id archiveId,part_index partIndex,filename,file_size fileSize,entry_count entryCount,manifest_fingerprint manifestFingerprint,connected,status,warning_count warningCount,sections FROM archive_parts ORDER BY part_index,id').map(archivePartFromRow);
...
function beginImportSession(sessionInput?: ImportSession, parts: ArchivePart[] = [], archiveSet?: NormalizedArchiveData['archiveSet'], archiveIdentity?: ArchiveIdentity) {
  ...
  db.transaction(() => {
    ...
    db.exec({ sql: 'INSERT OR IGNORE INTO archive_sets(id,platform,created_at,part_count,total_size,fingerprint,imported_at,status) VALUES(?,?,?,?,?,?,?,?)', bind: [set.id, set.platform, set.createdAt, set.partCount, set.totalSize, set.fingerprint, null, 'incomplete'] });
    ...
    for (const part of parts) db.exec({ sql: 'INSERT OR REPLACE INTO archive_parts(id,archive_id,part_index,filename,file_size,entry_count,manifest_fingerprint,connected,status,warning_count,sections) VALUES(?,?,?,?,?,?,?,?,?,?,?)', bind: [part.id, set.id, part.partIndex, part.filename, part.fileSize, part.entryCount, part.manifestFingerprint, part.connected ? 1 : 0, part.status ?? 'pending', part.warningCount ?? 0, JSON.stringify(part.sections ?? [])] });
  });
...
function rebuildSearchFromTables(): RebuildResult {
  ...
  const documents: unknown[][] = [], ftsDocuments: unknown[][] = [];
  const flush = () => { batchInsert('search_documents', ['entity_type', 'entity_id', 'title', 'body', 'context', 'created_at', 'conversation_id', 'source_path'], documents.splice(0), 'OR REPLACE'); if (searchBackend === 'fts5') batchInsert('archive_fts', ['entity_type', 'entity_id', 'title', 'body', 'context', 'created_at', 'conversation_id', 'source_path'], ftsDocuments.splice(0)); };
  const insert = (type: string, id: string, title?: string, body?: string, context?: string, createdAt?: string, conversationId?: string, source?: string) => { documents.push([type, id, title ?? null, body ?? null, context ?? null, createdAt ?? null, conversationId ?? null, source ?? null]); if (searchBackend === 'fts5') ftsDocuments.push([type, id, title ?? '', body ?? '', context ?? '', createdAt ?? '', conversationId ?? '', source ?? '']); if (documents.length >= SQLITE_BATCH_SIZE) flush(); };
  db.transaction(() => {
    rows('SELECT id,display_name displayName,username,relationship,facebook_id facebookId,identity_source identitySource,source_paths sourcePaths FROM people').forEach(row => insert('person', String(row.id), String(row.displayName), [row.username, row.relationship].filter(Boolean).join(' '), row.facebookId ? String(row.facebookId) : undefined, undefined, undefined, row.identitySource ? String(row.identitySource) : json<string[]>(row.sourcePaths, [])[0]));
    rows('SELECT id,category,label,value,start_date startDate,end_date endDate,source_path sourcePath FROM profile_facts').forEach(row => insert('profile-fact', `fact:${String(row.id)}`, String(row.category), [row.label, row.value].filter(Boolean).join(' '), [row.startDate, row.endDate].filter(Boolean).join(' '), row.startDate ? String(row.startDate) : row.endDate ? String(row.endDate) : undefined, undefined, String(row.sourcePath)));
    rows('SELECT id,title,body,created_at createdAt,source_path sourcePath FROM posts').forEach(row => insert('post', String(row.id), row.title ? String(row.title) : undefined, row.body ? String(row.body) : undefined, row.createdAt ? String(row.createdAt) : undefined, row.createdAt ? String(row.createdAt) : undefined, undefined, String(row.sourcePath)));
    rows('SELECT id,post_id postId,author_name authorName,body,created_at createdAt,source_path sourcePath FROM comments').forEach(row => insert('comment', String(row.id), row.authorName ? String(row.authorName) : undefined, String(row.body), String(row.postId), row.createdAt ? String(row.createdAt) : undefined, undefined, String(row.sourcePath)));
    rows('SELECT id,target_type targetType,target_id targetId,person_name personName,kind,created_at createdAt,source_path sourcePath FROM reactions').forEach(row => insert('reaction', String(row.id), String(row.kind), row.personName ? String(row.personName) : undefined, String(row.targetId), row.createdAt ? String(row.createdAt) : undefined, undefined, String(row.sourcePath)));
    rows('SELECT id,display_name displayName,relationship_type relationshipType,username,started_at startedAt,ended_at endedAt,source_path sourcePath FROM connections').forEach(row => insert('connection', String(row.id), String(row.displayName), [row.relationshipType, row.username].filter(Boolean).join(' '), row.startedAt ? String(row.startedAt) : row.endedAt ? String(row.endedAt) : undefined, row.startedAt ? String(row.startedAt) : row.endedAt ? String(row.endedAt) : undefined, undefined, String(row.sourcePath)));
    rows('SELECT id,title,description,created_at createdAt,updated_at updatedAt,source_path sourcePath FROM albums').forEach(row => insert('album', String(row.id), String(row.title), row.description ? String(row.description) : undefined, row.createdAt ? String(row.createdAt) : undefined, row.updatedAt ? String(row.updatedAt) : row.createdAt ? String(row.createdAt) : undefined, undefined, String(row.sourcePath)));
    rows('SELECT id,path,filename,caption,timestamp,source_path sourcePath FROM media').forEach(row => insert('media', String(row.id), row.filename ? String(row.filename) : String(row.path), row.caption ? String(row.caption) : undefined, row.path ? String(row.path) : undefined, row.timestamp ? String(row.timestamp) : undefined, undefined, String(row.sourcePath)));
    rows('SELECT id,title,participant_names participantNames,source_path sourcePath FROM conversations').forEach(row => insert('conversation', String(row.id), row.title ? String(row.title) : 'Conversation', String(row.participantNames ?? ''), String(row.participantNames ?? ''), undefined, undefined, String(row.sourcePath)));
    rows('SELECT m.id,m.conversation_id conversationId,m.sender_name senderName,m.body,m.sent_at sentAt,c.title,c.source_path sourcePath FROM messages m LEFT JOIN conversations c ON c.id=m.conversation_id').forEach(row => insert('message', String(row.id), row.title ? String(row.title) : 'Conversation', row.body ? String(row.body) : undefined, [row.senderName, row.sentAt].filter(Boolean).join(' '), row.sentAt ? String(row.sentAt) : undefined, String(row.conversationId), String(row.sourcePath ?? '')));
    flush();
  });
  db.exec('PRAGMA optimize');
...
function rebuildActivityIndex(): RebuildResult {
  const data = normalizedData({
    profile: (() => { const row = rows('SELECT id,person_id personId,display_name displayName,username,bio,joined_at joinedAt,source_path sourcePath FROM profiles LIMIT 1')[0]; return row ? { id: String(row.id), personId: String(row.personId), displayName: String(row.displayName), username: row.username ? String(row.username) : undefined, bio: row.bio ? String(row.bio) : undefined, joinedAt: row.joinedAt ? String(row.joinedAt) : undefined, source: { platform: 'facebook', path: String(row.sourcePath) } } as Profile : undefined; })(),
    people: [], posts: rows('SELECT id,author_id authorId,title,body text,created_at createdAt,source_path sourcePath FROM posts').map(row => ({ id: String(row.id), authorId: row.authorId ? String(row.authorId) : undefined, title: row.title ? String(row.title) : undefined, text: row.text ? String(row.text) : undefined, createdAt: row.createdAt ? String(row.createdAt) : undefined, source: { platform: 'facebook', path: String(row.sourcePath) } } as Post)),
    comments: rows('SELECT id,post_id postId,author_id authorId,author_name authorName,body text,created_at createdAt,source_path sourcePath FROM comments').map(row => ({ id: String(row.id), postId: String(row.postId), authorId: row.authorId ? String(row.authorId) : undefined, authorName: row.authorName ? String(row.authorName) : undefined, text: String(row.text), createdAt: row.createdAt ? String(row.createdAt) : undefined, source: { platform: 'facebook', path: String(row.sourcePath) } } as Comment)),
    reactions: rows('SELECT id,target_type targetType,target_id targetId,person_id personId,person_name personName,kind,created_at createdAt,source_path sourcePath FROM reactions').map(row => ({ id: String(row.id), targetType: String(row.targetType) as Reaction['targetType'], targetId: String(row.targetId), personId: row.personId ? String(row.personId) : undefined, personName: row.personName ? String(row.personName) : undefined, kind: String(row.kind), createdAt: row.createdAt ? String(row.createdAt) : undefined, source: { platform: 'facebook', path: String(row.sourcePath) } } as Reaction)),
    connections: rows('SELECT id,person_id personId,display_name displayName,relationship_type type,started_at startedAt,ended_at endedAt,source_path sourcePath FROM connections').map(row => ({ id: String(row.id), personId: String(row.personId), displayName: String(row.displayName), type: String(row.type) as Connection['type'], startedAt: row.startedAt ? String(row.startedAt) : undefined, endedAt: row.endedAt ? String(row.endedAt) : undefined, source: { platform: 'facebook', path: String(row.sourcePath) } } as Connection)),
    albums: rows('SELECT id,title,created_at createdAt,updated_at updatedAt,source_path sourcePath FROM albums').map(row => ({ id: String(row.id), title: String(row.title), createdAt: row.createdAt ? String(row.createdAt) : undefined, updatedAt: row.updatedAt ? String(row.updatedAt) : undefined, mediaIds: [], source: { platform: 'facebook', path: String(row.sourcePath) } } as Album)),
    media: rows('SELECT id,filename,path,timestamp,owner_type ownerType,owner_id ownerId,source_path sourcePath FROM media').map(row => ({ id: String(row.id), filename: row.filename ? String(row.filename) : undefined, path: String(row.path), timestamp: row.timestamp ? String(row.timestamp) : undefined, ownerType: String(row.ownerType) as Media['ownerType'], ownerId: String(row.ownerId), mediaType: 'unknown', source: { platform: 'facebook', path: String(row.sourcePath) } } as Media)), warnings: []
  });
  ...
  const records = activityForData(data); db.transaction(() => { db.exec('DELETE FROM activity_records'); batchInsert('activity_records', ['id', 'activity_type', 'actor_person_id', 'target_type', 'target_id', 'occurred_at', 'calendar_month', 'calendar_day', 'calendar_year', 'summary', 'source_path', 'source_index', 'archive_part_id'], records.map(activity => { const parts = calendarParts(activity.timestamp); return [activity.id, activity.type, activity.actorPersonId ?? null, activity.targetType ?? null, activity.targetId ?? null, activity.timestamp, parts.month, parts.day, parts.year, activity.summary, activity.source.path, activity.source.index ?? null, activity.source.archivePartId ?? null]; }), 'OR IGNORE'); });
  return { kind: 'activity', rows: records.length, completedAt: new Date().toISOString() };
}
...
self.onmessage = async (event: MessageEvent<DatabaseRequest>) => {
  ...
  try {
    ...
    if (request.type === 'import-part' && request.data && request.part && request.sessionId) { const checkpoint = importPartData(request.data, request.part, request.sessionId); await persistFallback(); send({ id: request.id, ok: true, data: checkpoint }); return; }
